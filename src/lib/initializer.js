...
export function activator(canvas, webGL, colorFormat, PROGRAMS, pointers, emitters) {
    if(active) {
        let nPointers = [];
    ...
    function input() {
        ...
        for(let e of emitters){
            if(e.emits) splat(e.x,e.y,e.dx,e.dy,e.color)
            if(e.isPlayOnce) e.stop()
        }
